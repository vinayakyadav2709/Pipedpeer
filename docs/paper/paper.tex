...
\title{PipedPeer: Transparent Distribution of Unmodified Python Workloads\\
over a Zero-Configuration Peer Network}
...
of the same environment took 0.66\,s, an eighteenfold reduction. Second, the
...
package set makes an environment's content-addressed name identical across
machines and across time, which is what allows a closure to be transferred once
per cluster rather than once per job (Section~\ref{sec:env}).
...
\item \textbf{Exact aggregation without combiners.} Hash-shuffling on the
grouping key places every key complete on one node, so any aggregation is exact,
including those with no combiner form (Section~\ref{sec:shuffle}).
...
\item \textbf{An unconditional degradation guarantee}, enforced structurally
rather than by error handling (Section~\ref{sec:ft}).
\end{enumerate}
...
their API. BOINC~\cite{anderson2004boinc} pioneered volunteer computing but is a
...
involved, and consequently \emph{any} aggregation specification is exact,
including user-defined reductions that admit no combiner. This is a stronger
guarantee than the partial-aggregation strategies used by systems that push
combiners to the workers.
...
Using the cluster must never be worse than not using it. Five independent layers
enforce this. Chunks of a parallel map are dealt alternately to the local side
...
median of 0.66\,s thereafter, an \textbf{eighteenfold reduction}
...
established by comparison against local execution: grouped aggregation and all
four join types are frame-equal to local pandas, and each rank's gradient in
data-parallel training is exactly the mean across ranks.
...
\footnotesize
\begin{tabularx}{\columnwidth}{X C{0.9cm} C{0.8cm} C{0.8cm} C{0.9cm}}
\toprule
\textbf{Property} & \textbf{Ours} & \textbf{Spark} & \textbf{Dask} & \textbf{ssh} \\
\midrule
\multicolumn{5}{l}{\textit{What it does differently}} \\
Runs unmodified Python     & yes & no  & partial & yes \\
No cluster to configure    & yes & no  & no      & partial \\
No head or master node     & yes & no  & no      & yes \\
Reproduces the environment & yes & partial & partial & no \\
Sandboxes each job         & yes & no  & no      & no \\
Survives losing a node     & yes & yes & yes     & no \\
\midrule
\multicolumn{5}{l}{\textit{Where it is behind}} \\
Authenticated, encrypted   & \textbf{no} & yes & partial & yes \\
Works beyond one LAN       & partial & yes & yes & yes \\
Runtimes beyond Python     & \textbf{no} & yes & no & yes \\
Proven in production       & \textbf{no} & yes & yes & yes \\
\bottomrule
\end{tabularx}
\end{table}
